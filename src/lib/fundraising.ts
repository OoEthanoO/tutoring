...
export const fetchFundraisingRaisedAmount = async () => {
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 8000);
    const response = await fetch(fundraiserCampaignUrl, {
      method: "GET",
      signal: controller.signal,
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; YanLearnBot/1.0; +https://learn.ethanyanxu.com)",
      },
      cache: "no-store",
    });
